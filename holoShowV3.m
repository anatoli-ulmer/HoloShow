...
% Last Modified by GUIDE v2.5 17-Aug-2016 11:37:14
...
if ~exist('goodShots','dir')
    mkdir('goodShots')
end
fileID=fopen(fullfile('goodShots',[foldername,'.txt']),'a');
fprintf(fileID,'%s\n',shotname);
fclose(fileID);
...
if get(handles.decon_checkbox,'value')
    ...
    %     handles.hologram.deconvoluted = handles.hologram.deconvoluted.*handles.mask;handles.hologram.deconvoluted;
    %     figure(23423); mi = max(log10(abs(handles.hologram.deconvoluted(:)))); colormap fire;
    %     imagesc(log10(abs(handles.hologram.deconvoluted)),[mi-4,mi]); axis square;
    %     figure(3242354); imagesc(handles.mask);
    %     handles.hologram.deconvoluted = handles.hologram.deconvoluted.*handles.mask;
    %     handles.hologram.deconvoluted = propagate(abs(handles.hologram.deconvoluted), handles.phase);
%     M = handles.mask;
    handles.recon=fftshift(ifft2(fftshift(handles.hologram.deconvoluted)));
    handles.recon=-handles.recon;
    ...
    Fref = Freference;
    Fref = Fref/sum(abs(Fref(:)));
    Imeas = handles.hologram.masked;
    Imeas = Imeas/sum(Imeas(:));
    decon = zeros(1024);
    decon(handles.rect(2):handles.rect(2)+handles.rect(4),handles.rect(1):handles.rect(1)+handles.rect(3)) = handles.recon(handles.rect(2):handles.rect(2)+handles.rect(4),handles.rect(1):handles.rect(1)+handles.rect(3));
    Fdecon = fftshift(fft2(fftshift(decon)));
    Fdecon = Fdecon./sum(abs(Fdecon(:)));
    
    k=1;
    
%     for q=-10:0.1:10
%         temp = abs(exp(q)*Fref + Fdecon).^2;
%         temp = temp/sum(temp(:));
%         figure(76); imagesc(log10(abs(Imeas-temp))); drawnow;
%         err_ampl(k) = sum(abs(Imeas(:))-temp(:));
%         k=k+1;
%     end
    
%     figure(986); plot(err_ampl);
...
function tais_pushbutton_Callback(hObject, eventdata, handles)
holo = handles.hologram.masked;
[X,Y] = meshgrid(-size(holo,1)/2:size(holo,1)/2-1,-size(holo,2)/2:size(holo,2)/2-1);

newFilter = X.^2+Y.^2;
figure(764654);
set(gcf, 'Name', 'Filter'); % gcf=get current figure
imagesc(newFilter); axis square; colorbar;
newHolo = handles.hologram.masked.*newFilter;
imagesc(log10(abs(newHolo)));axis square; colorbar;

newHolo = propagate(abs(newHolo), handles.phase);
newHolo = newHolo.*exp(1i*handles.phaseOffset);

handles.reconNew=fftshift(ifft2(fftshift(newHolo)));

figure(234468);
imagesc(part_and_scale(handles.reconNew(handles.rect(2):handles.rect(2)+handles.rect(4),handles.rect(1):handles.rect(1)+handles.rect(3)),...
    handles.logSwitch, handles.partSwitch));
axis square;
colormap gray;
...
h=get(0,'Children');
save_images(h,handles.currentFile);


function damn_pushbutton_Callback(hObject, eventdata, handles)

for i=1:1%size(handles.filenames,2)
    set(handles.scale_checkbox, 'Value', false)
    handles.fileIndex = i; % get index of selection
    handles = select_hologram(hObject, eventdata, handles); % GRAB HOLOGRAM
    fprintf('please choose noise \n');
    noise_pushbutton_Callback(hObject, eventdata, handles);
    set(handles.filenames_listbox, 'Value', handles.fileIndex);
    try
        handles = select_hologram(hObject, eventdata, handles);
    catch
    ...
    for CC=1:nbrCCs
        set(handles.decon_checkbox, 'Value', false)
        ...
        fprintf('please choose deconvolution radius and press enter \n');
        set(handles.decon_checkbox, 'Value', true);
        decon_checkbox_Callback(hObject, eventdata, handles);
        pause
        save_image_pushbutton_Callback(hObject, eventdata, handles);
    end
    
    %     do_it_yourself(hObject, eventdata, handles, ...
...
function showDHS_pushbutton_Callback(hObject, eventdata, handles)
DHSfile = fullfile(handles.pathname, '..', ['hitScore_', handles.pathname(end-5:end-1), '.dat']);
handles.runDHS = dlmread(DHSfile);
figure(61);
plot(handles.runDHS(handles.runDHS>200) )
