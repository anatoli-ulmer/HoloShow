...
switch app.handles.ext
    ...
    case '.mat'
        ...
        if strcmp(app.handles.currentFile(7:11), 'frame')
            ...
            % HARD CODED MASKING FOR LCLS2014 DATA
    ...
    case '.cxi'
        app.handles.hologram.orig = h5read(fullfile(app.handles.pathname, app.handles.first_file), app.handles.h5path.pnCCD, [1 1 app.handles.fileIndex],[1024 1024 1]);
        try
            app.handles.hummingbird_mask = h5read(fullfile(app.handles.pathname, app.handles.first_file), app.handles.h5path.mask);
        catch
    ...
    case '.h5'
        app.handles.hologram.orig = h5read(fullfile(app.handles.pathname, app.handles.first_file), app.handles.h5path.pnCCD, [1+app.handles.img_offset 1 app.handles.fileIndex],[1074 1024 1]);
        try
            app.handles.hummingbird_mask = h5read(fullfile(app.handles.pathname, app.handles.first_file), app.handles.h5path.mask, [1+app.handles.img_offset 1],[1074 1024]);
        catch
            warning('no hummingbird mask found!')
            app.handles.hummingbird_mask = ones(size(app.handles.hologram.orig));
        end
